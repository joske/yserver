...
/// Exploratory end-to-end: the real core loop on `KmsBackend::for_tests`
/// behind `/tmp/.X11-unix/X95`, driven by the real `xmodmap` and `xdotool`
/// binaries as `tools/vng-scenarios/xmodmap-caps-control.sh` does. Needs
...
fn xmodmap_then_xdotool_against_the_core_loop() {
    ...
    let after = run(&["xdotool", "keydown", "66", "key", "a", "keyup", "66"]);
    sender.send(Message::Shutdown).unwrap();
    let _ = handle.join();
    let _ = fs::remove_file(&path);
    assert!(before.0, "xdotool before xmodmap: {}", before.1);
    assert!(xmodmap.0, "xmodmap: {}", xmodmap.1);
    assert!(after.0, "xdotool after xmodmap: {}", after.1);
}
