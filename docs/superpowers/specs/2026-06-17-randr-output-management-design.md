...
- **The registry is authoritative for position/mode; the rescan path must stop overwriting it.** `recompact_horizontal_layout` is the **boot/default** extend-right helper only. Today `requery_outputs_and_modeset` calls it **unconditionally** after every rescan (`platform.rs:2351`) and both resume and display-rescan run through that path (`backend.rs:4784,4949`), so a client's `(x,y)` (and any non-default mode) would be flattened on the next hotplug or VT resume. This must change: an output that has a client-set config in the registry keeps its `(x,y,mode)` verbatim across rescan/resume; `recompact_horizontal_layout` runs only for outputs that have **never** been client-configured (the boot default / a brand-new connector before any `SetCrtcConfig`). Concretely: drop the unconditional recompact from the rescan path and re-apply each enabled output's registry config instead. The VT-resume relight (`dpms_set_outputs_active(true)`, already on the branch) likewise re-commits the registry's stored mode, not a fresh auto-layout.
...
- **`GetMonitors` (RANDR 1.5) / XINERAMA** (`active_monitors`, `process_request.rs:2085/2341`): yserver has no manual `RRSetMonitor`/`DeleteMonitor` (out of scope), so the monitor set always derives from outputs. **Honor the `get_active` flag** (Xorg filters zero-area only under `get_active=true`, `rrmonitor.c:247,343,590`): `get_active=true` (XINERAMA + the path MATE/marco read for per-monitor placement/maximize/panel geometry) → one monitor per **enabled** output at its client-set `(x,y,width,height)`, off/disconnected (zero-area) **excluded**; `get_active=false` → all **connected** outputs as monitors (off ones reported at zero geometry). XINERAMA's `QueryScreens` maps to the `get_active=true` set. It must track the client-set layout, not the raw boot order.
...
- `GetMonitors`: `get_active=true` excludes off/disconnected; `get_active=false` includes connected-but-off outputs at zero geometry.
