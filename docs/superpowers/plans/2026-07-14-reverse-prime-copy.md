...
3. Add minimal per-sink Vulkan transfer contexts, one exact explicit
   `DRM_FORMAT_MOD_LINEAR` A-to-B transport plan paired with every B-local
   destination plan, and independent optimal A render targets without touching
   the TFP strategy cache;
...
6. Render into A's optimal target, linearize into the external transport,
   ...
   page flip.
7. Track exclusive FOREIGN ownership and layout provenance for the linear A/B
